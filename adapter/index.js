...
const plugin = function (defaults = {}) {
    ...
    return {
        ...
        async adapt(builder) {
                ...
                try {
                    ...
                    if (builder.hasServerInstrumentationFile?.()) {
                        instrumentation_result = await esbuild.build({
                            entryPoints: [
                                `${builder.getServerDirectory()}/instrumentation.server.js`,
                            ],
                    ...
                    const warnings = instrumentation_result
                        ? [
                              ...result.warnings,
                              ...instrumentation_result.warnings,
                          ]
                        : result.warnings;
                    ...
                    const error =
                        /** @type {import('esbuild').BuildFailure} */ (err);
            ...
            for (const route of builder.routes) {
                ...
                if (isr) {
                    ...
                    const json = JSON.stringify(
                        {
                            ...isr,
                            expiration: parse_isr_expiration(
                                isr.expiration,
                                route.id,
                            ),
                        },
                    ...
                    const data_symlink_path = path.join(
                        base_dir,
                        "__data.json.func",
                    ); // e.g., .vercel/output/functions/index/__data.json.func
            ...
            write(
                `${dir}/config.json`,
                JSON.stringify(static_config, null, "\t"),
            );
        },
...
async function create_function_bundle(builder, entry, dir, config) {
    ...
    traced.warnings.forEach((error) => {
        ...
        if (error.message.startsWith("Failed to resolve dependency node:"))
            return;
        ...
        if (error.message.startsWith("Failed to resolve dependency")) {
            ...
            /** @type {string[]} */ (resolution_failures.get(importer)).push(
                module,
            );
        } else {
            throw error;
